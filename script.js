...
let initialBoard = [
    ...
    "", "", "", "", "", "", "", "",
    pawn, pawn, pawn, pawn, pawn, pawn, pawn, pawn,
    fortress, horse, bishop, king, queen, bishop, horse, fortress
];
...
class Board {
    createBoard() {
        initialBoard.forEach((piece, i) => {
            ...
            if (row % 2 == 0) {
                square.classList.add(i % 2 == 0 ? "yellow" : "green");
            } else {
                square.classList.add(i % 2 == 0 ? "green" : "yellow");
            }
...
class Control {
    ...
    dragDrop(e) {
        ...
        let correctGo = targetElement.firstChild.classList.contains(playerGo);
        let taken = e.target.classList.contains("piece");
        let opponentGo = playerGo === 'red' ? 'black' : 'red';
        let takenOpponent = e.target.firstChild?.classList.contains(opponentGo);
        let valid = this.rule.checkIfValid(e.target);

        if (correctGo) {
            if (takenOpponent && valid) {
                e.target.parentNode.append(targetElement);
                e.target.remove();
                this.rule.changePlayer();
                return;
            }

            if (taken && !takenOpponent) {
                infoDisplay.innerHTML = "Kamu tidak bisa bergerak ke situ";
                setTimeout(() => {
...
class Rule {
    changePlayer() {
        if (playerGo == "red") {
            playerGo = "black";
            playerTwo.innerHTML = "Player 2 main";
            playerOne.innerHTML = "Menunggu Giliran Main";
            this.revertIds();
        } else {
            playerGo = "red";
            playerOne.innerHTML = "Player 1 main";
    ...
    checkIfValid(target) {
        ...
        let startId = Number(positionId);
        let piece = targetElement.id;
